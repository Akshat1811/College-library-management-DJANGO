...
def admin_home(request):
    return render(request,'../templates/admin_home.html')
@login_required
...
def admin_req(request):
    ...
    books1 = Request.objects.filter(User__in=users).values_list('Book',flat=True)
    books = Books.objects.filter(Book_id__in=books1)
    context = {
        'books':books
    }
